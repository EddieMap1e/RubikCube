...
bool BasicEffect::InitAll(ComPtr<ID3D11Device> device)
{
	...
	// 创建顶点着色器
	device->CreateVertexShader(g_Basic_VS, sizeof(g_Basic_VS), nullptr, pImpl->basicVS.GetAddressOf());
	// 创建顶点布局
	...
	// 创建像素着色器
	device->CreatePixelShader(g_Basic_PS, sizeof(g_Basic_PS), nullptr, pImpl->basicPS.GetAddressOf());
	...
	device->CreateSamplerState(&sd, pImpl->ssLinearWrap.GetAddressOf());
